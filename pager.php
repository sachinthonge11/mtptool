...
$curr_page= array_search($pageid,$pagearr);
...
<?php if($prev_page){?>
    <a href="pager.php?pageurlid=<?php echo $pagearr[$prev_page]; ?>"> PREV </a>
<?php }    ?>
<?php /*for($i = 1; $i <= $qty_pages; $i++){ 

        //echo  "value of itemes=".$items[$i];
    ?>
    <a href="pager.php?pageurlid=<?php echo $pagearr[$i]; ?>" class="<?php ($i == $curr_page) ? 'curr' : '' ?>"><?php echo $i; ?></a>
<?php }*/?>
<?php if($next_page){ ?>
        <a href="pager.php?pageurlid=<?php echo $pagearr[$next_page]; ?>"> NEXT </a>
<?php } ?>
